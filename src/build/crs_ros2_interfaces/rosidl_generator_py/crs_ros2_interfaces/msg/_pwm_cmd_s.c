...
bool crs_ros2_interfaces__msg__pwm_cmd__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // is_overriding
    PyObject * field = PyObject_GetAttrString(_pymsg, "is_overriding");
    if (!field) {
      return false;
    }
    assert(PyBool_Check(field));
    ros_message->is_overriding = (Py_True == field);
    Py_DECREF(field);
  }
...
PyObject * crs_ros2_interfaces__msg__pwm_cmd__convert_to_py(void * raw_ros_message)
{
  ...
  {  // is_overriding
    PyObject * field = NULL;
    field = PyBool_FromLong(ros_message->is_overriding ? 1 : 0);
    {
      int rc = PyObject_SetAttrString(_pymessage, "is_overriding", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
